...
app.use((ctx, next) => {
  ...
  ctx.response.set("Access-Control-Allow-Origin", "*"); //позволяет веб-страницам получить доступ к ресурсам другого домена
  ctx.response.set('Access-Control-Allow-Methods', 'DELETE, PUT, PATCH, GET, POST'); //использовать разрешенные методы
